...
export function actFetchArticleGeneral({ currentPage, posts,total,totalPages }) {
  return {
    type: ACT_FETCH_ARTICLE_GENERAL,
    payload: {
      currentPage, posts,total,totalPages
    },
  };
}
...
export function actFetchArticleGeneralAsync({
  ...
  return async (dispatch) => {
    try {
      ...
      dispatch(actFetchArticleGeneral({ currentPage, posts,total,totalPages }));
    } catch (error) {}
  };
}
